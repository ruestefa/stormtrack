...
c     Input constants file
      character*80                        inp_const      
...
c     Format of front data      
      character*80                        inpformat
      parameter                           (inpformat='compress')
c      parameter                           (inpformat='2d')
...
c     Grid parameters and 2d fields
      ...
      integer,allocatable, dimension (:,:) :: field_flag,field_tmp
      integer,allocatable, dimension (:,:) :: cluster
      integer                                 ncluster
      character*80                            fieldname
      real,allocatable, dimension (:)      :: inp_lon,inp_lat
      real,allocatable, dimension (:)      :: inp_farea,inp_fvel
      integer                                 nlist
...
c     Read in parameter file
      ...
      print*,'Constants     : ',trim(inp_const)
      ...
      print*,'Inp format    : ',trim(inpformat)

c     -------------------------------------------------------------------
c     Set grid parameters + allocate memory
c     -------------------------------------------------------------------

c     Open file
      ierror = NF90_OPEN(TRIM(inp_const),nf90_nowrite,cdfid)
...
c     Allocate memory for input and output field
      ...
      allocate(field_tmp (nx,ny),stat=stat)
      if (stat.ne.0) print*,'*** error allocating array field_tmp  ***'
...
c     Open file
      ierror = NF90_OPEN(TRIM(inp_fronts),nf90_nowrite,cdfid)
      IF ( ierror /= nf90_NoErr ) PRINT *,NF90_STRERROR(ierror)

c     Read data - dependimg on format
      if ( inpformat.eq.'2d' ) then
          varname = 'fflag_'//trim(fieldname)
          ierror = nf90_inq_varid(cdfid,varname, varid)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          ierror = NF90_GET_VAR(cdfid,varid,field_the)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          print*,'R  fflag_'//trim(fieldname)

          varname = 'fvel_'//trim(fieldname)
          ierror = nf90_inq_varid(cdfid,varname, varid)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          ierror = NF90_GET_VAR(cdfid,varid,field_fvel)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          print*,'R  fvel_'//trim(fieldname)
          
      elseif ( inpformat.eq.'compress' ) then
          ierror = nf90_inq_dimid(cdfid,'list',dimid)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          ierror = nf90_inquire_dimension(cdfid, dimid, len = nlist)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          
          allocate(inp_lon  (nlist))
          allocate(inp_lat  (nlist))
          allocate(inp_farea(nlist))
          allocate(inp_fvel (nlist))
          
          varname = 'lon'
          ierror = nf90_inq_varid(cdfid,varname, varid)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          ierror = NF90_GET_VAR(cdfid,varid,inp_lon)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          print*,'R  lon'
          varname = 'lat'
          ierror = nf90_inq_varid(cdfid,varname, varid)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          ierror = NF90_GET_VAR(cdfid,varid,inp_lat)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          print*,'R  lat'
          varname = 'farea_'//trim(fieldname)
          ierror = nf90_inq_varid(cdfid,varname, varid)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          ierror = NF90_GET_VAR(cdfid,varid,inp_farea)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          print*,'R  farea_'//trim(fieldname)
          varname = 'fvel_'//trim(fieldname)
          ierror = nf90_inq_varid(cdfid,varname, varid)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          ierror = NF90_GET_VAR(cdfid,varid,inp_fvel)
          IF(ierror /= nf90_NoErr) PRINT *,NF90_STRERROR(ierror)
          print*,'R  fvel_'//trim(fieldname)
          ...
          do i=1,nlist
             indx = nint( ( inp_lon(i) - xmin ) / dx + 1. )
             indy = nint( ( inp_lat(i) - ymin ) / dy + 1. )
             field_the (indx,indy) = inp_farea(i)
             field_fvel(indx,indy) = inp_fvel (i)
          enddo
...
c             Accept it if only one neighbour found
              if ( icount.eq.1 ) then
                 indx = i
                 indy = j
                 print*,'One-neighbour starting point ',indx,indy
                 goto 110
              endif
...
c             Accept it if only one neighbour was found found
              if ( icount.eq.1 ) then
                 indx = i
                 indy = j
                 print*,'Two-neighbour starting point ',indx,indy
                 goto 110
              endif
      ...
      do i=1,nx
        do j=1,ny
            if ( field_flag(i,j).ne.0 ) then
               indx = i
               indy = j
                print*,'Arbitrary starting point ',indx,indy
               goto 110
            endif
         enddo
      enddo
...
c     -------------------------------------------------------------------
      ...
      print*
      print*,"----- REMOVE SHORT LINE SEGMENTS ------------------------"
      print*
...
c     -------------------------------------------------------------------
      ...
      print*
      print*,"----- DEFINE NEW THE AND VEL FIELDS ---------------------"
      print*
