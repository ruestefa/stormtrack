...
cdef np.ndarray[np.float32_t, ndim=4] _pairwise_distances_great_circle__core(
    ...
    cdef int nx=lon.shape[0]
    cdef int ny=lon.shape[1]
    cdef np.ndarray[np.float32_t, ndim=4] dists = np.empty([nx, ny, nx, ny], np.float32)
    cdef np.float32_t EARTH_CIRC = 6378137
    cdef np.float32_t PI180 = M_PI/180
    cdef np.float32_t lon0
    cdef np.float32_t lat0
    cdef np.float32_t lon1
    cdef np.float32_t lat1
    cdef np.float32_t dist
    cdef np.float32_t dlon
    cdef np.float32_t dlat
    cdef np.float32_t a
    cdef np.float32_t c
    cdef int i0
    cdef int j0
...
def closest_feature(features, lon=None, lat=None, great_circle=True, *,
    ...
    # Compute distances
    if not great_circle:
        _compute_feature_distances_xy(dists, inds_shell, mask_features, mask, nx, ny)
    elif approx:
        _compute_feature_distances_lonlat_approx(
            dists, inds_shell, mask_features, mask, lon, lat,
        )
    else:
        _compute_feature_distances_lonlat_exact(
            dists, inds_shell, mask_features, mask, lon, lat,
        )
...
def _compute_feature_distances_lonlat_exact(
    ...
    cdef int nx = dists.shape[0]
    cdef int ny = dists.shape[1]
    cdef int ns = inds_shell.size / 2
    cdef np.float32_t mindist
    cdef np.float32_t dist
    cdef int i0
    cdef int j0
    cdef int k
...
def _compute_feature_distances_lonlat_approx(
    ...
    cdef np.float32_t lon1
    cdef np.float32_t lat1
    cdef np.float32_t minlon0
    cdef np.float32_t minlat0
    cdef np.float32_t minlon1
    cdef np.float32_t minlat1
    for i0 in range(nx):
        for j0 in range(ny):
            if not mask[i0, j0]:
                continue
