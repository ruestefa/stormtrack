...
def identify_features(fld, feature_name, nx, ny, lower, upper, minsize,
        ...
        split_seed_minstrength, topo_filter_apply, topo_filter_mode,
        topo_filter_threshold, topo_filter_min_overlap, topo_fld,
        silent=False, grid=None,
    ):
    ...
    try:
        ...
        err = "error identifying features: {}({})".format(
                e.__class__.__name__, e)
        raise Exception(err)
    if not silent:
        log.info("identified {} '{}' features based on range ({}..{})".format(
                len(features), feature_name, lower, upper))
    ...
    if split_levels is not None:
        ...
        if not silent:
            log.info("split {} '{}' features into {} based on levels {}".format(
                    nold, feature_name, len(features), split_levels))

    # Add properties
    if debug: log.debug("add properties to '{}' features".format(feature_name))
...
def find_features_2d_threshold_seeded(
    ...
    if lower is None and upper is None:
        err = "No threshold given! Must pass lower, upper, or both!"
    ...
    if seeds is not None:
        if random_seed is not None:
            log.warning("inconsistent arguments: "
                    "random_seed pointless when passing seeds")
...
cdef list _find_features_threshold_random_seeds(
    ...
    cdef np.ndarray[np.int32_t, ndim=1] dummy_center = np.array(
                [], dtype=np.int32)
    cdef np.ndarray[np.int32_t, ndim=2] dummy_extrema = np.array(
                [[]], dtype=np.int32)
    ...
    cdef np.int32_t** random_seeds = <np.int32_t**>malloc(
                n_seeds*sizeof(np.int32_t*))
...
cdef list c_find_features_2d_threshold_seeds(
    ...
    cdef np.int32_t** seeds = <np.int32_t**>malloc(
                n_seeds*sizeof(np.int32_t*))
    for i in range(n_seeds):
...
cdef void c_find_features_2d_threshold_seeds_core(
    ...
    if debug: log.debug("< c_find_features_2d_threshold_seeds_core ({} seeds)".format(n_seeds))
    cdef int i, j, pid, i_seeds=0
    cdef int nx=field_raw.shape[0], ny=field_raw.shape[1]
    ...
    if debug: log.debug("n_unassigned_seeds0={}".format(n_unassigned_seeds))
    if debug: log.debug("n_unassigned_pixels0={}".format(n_unassigned_pixels[0]))
    ...
    for i in range(n_iter_max):
        ...
        if i_seeds == n_seeds:
            if debug: log.debug("BREAK i_seeds == n_seeds ({})".format(i_seeds))
            break
        ...
        err = "identify_regions: loop timed out (nmax={})".format(n_iter_max)
        raise Exception(err)
...
cdef void grow_cregion_rec(
    ...
    if debug:
        ...
        for i in range(cregions.n):
            log.debug(" => [{}] {}/{}, {}/{}".format(cregions.regions[i].id,
                        cregions.regions[i].pixels_n,
                        cregions.regions[i].pixels_max,
    ...
    for i in range(cpixel_center.neighbors_max):
        ...
        if debug: log.debug("OTHER PIXEL: ({}, {}) [{}/{}]".format(other_cpixel.x, other_cpixel.y, i, cpixel_center.neighbors_max))

        if ctype == pixeltype_background:
            if debug: log.debug(" -> other pixel is background: ({}, {})".format(other_cpixel.x, other_cpixel.y))
            continue
            ...
            if other_cpixel.region != cpixel_center.region:
                if debug: log.debug("< connect {} {}".format(cpixel_center.region.id, other_cpixel.region.id))
                cregions_connect(
            ...
            if debug: log.debug(" -> other pixel is unassigned: ({}, {})".format(other_cpixel.x, other_cpixel.y))
...
cdef cRegions cregions_merge_connected(
    ...
    cdef bint debug = False
    if debug: log.debug("< cregions_merge_connected")
    ...
    cdef cRegion** cregions_todo = <cRegion**>malloc(
            cregions.n*sizeof(cRegion*))
    for i in range(cregions.n):
...
cdef void _cregions_merge_connected_core(
    ...
    cdef int iter_i, iter_max=1000000
    for iter_i in range(iter_max):
        if debug: log.debug("\n+++ ITER {} +++".format(iter_i))
        for i in range(cregions_n):
            if cregions_todo[i] is not NULL:
                if debug: log.debug("cregions.regions[{}] is not NULL".format(i))
                break
        else:
            if debug: log.debug("BREAK all {} regions processed".format(cregions_n))
            break
        ...
        if debug: log.debug("connected_regions.n={}".format(connected_regions_n))
        ...
        if debug: log.debug("n_pixels_feature={}".format(n_pixels_feature))
        ...
        err = "loop timed out after {} iterations".format(iter_i)
        raise Exception(err)
...
cdef int collect_pixels(
    ...
    cdef int i, j, k, i_pixels_feature=0
...
cdef void _cregion_collect_connected_regions_rec(
    ...
    if connected_regions_n[0] == connected_regions_max:
        err = ("_cregion_collect_connected_regions_rec: connected_regions not big enough (max={})"
                ).format(connected_regions_max)
        raise Exception(err)
...
cdef void assign_cpixel(
    ...
    if cpixel.v < lower_threshold or cpixel.v >= upper_threshold:
        ...
        if debug: log.debug("> assign_cpixel ({}, {}) BACKGROUND".format(cpixel.x, cpixel.y))
        return
...
def find_features_2d_threshold(
    ...
    if lower >= 0 and upper >= 0:
        ...
        err = "no threshold given (neither lower nor upper)"
        raise ValueError(err)
    ...
    # Identify regions
    if debug and mode == -1: log.debug("identify regions (..{})".format(lower, upper))
    if debug and mode ==  0: log.debug("identify regions ({}..{})".format(lower, upper))
    if debug and mode ==  1: log.debug("identify regions ({}..)".format(lower, upper))
    ...
    for i in range(cconstants.nx):
        for j in range(cconstants.ny):
            ...
            if cregion is NULL:
                ...
                cregions_link_region(&cregions, cregion, cleanup=False, unlink_pixels=False)
                if debug: log.debug("new region {}".format(cregion.id))
            if debug: log.debug("[{}] ({}, {}) {}".format(cregion.id, cpixel.x, cpixel.y, cpixel.v))
            cregion_insert_pixel(
                    cregion,
                    cpixel,
                    link_region=True,
    ...
    if cgrid.pixel_status_table is NULL:
        ...
        pixel_status_table_reset(cgrid.pixel_status_table, cgrid.constants.nx, cgrid.constants.ny)
    ...
    if debug: log.debug("create feature objects")
    cdef bint ignore_missing_neighbors = False
    cdef list features = cregions_create_features(
            &cregions,
            base_id,
...
cdef inline cRegion* find_existing_region(
    ...
    if loop_order != loop_order_ij and loop_order != loop_order_ji:
        err = ("invalid loop order '{}' (must be 0=ij or 1=ji)"
                ).format(loop_order)
        raise Exception(err)
    ...
    for i in range(3, 8):
        ...
        if debug: log.debug("{} cregion: {}, cregion_i: {}".format(i, "NULL" if cregion_i is NULL else cregion.id, "NULL" if cregion_i is NULL else cregion_i.id))
        if cregion_i is not NULL:
            if cregion is NULL:
                cregion = cregion_i
            elif cregion.id != cregion_i.id:
                if debug: log.debug(" -> merge cregions {} and {}".format(cregion.id, cregion_i.id))
                cregion = cregion_merge(
                        cregion,
                        cregion_i,
                    )
...
cpdef list split_regiongrow_points(
        Feature feature,
        np.ndarray[np.int32_t, ndim=2] seed_points,
        int nx = 0,
        int ny = 0,
        np.uint64_t base_id = 0,
        Constants constants = None,
    ...
    # Run splitting
    cdef cRegions subfeature_cregions = _split_regiongrow_points_core(
            &cfeature, seed_points, &grid)

    # Turn subfeature cregions into Feature objects
...
cdef cRegions _split_regiongrow_points_core(
    ...
    cdef int n_seed_points = len(seed_points)
    cdef cRegions cregions_seeds = cregions_create(n_seed_points)
    cdef cPixel* cpixels_seeds = <cPixel*>malloc(n_seed_points*sizeof(cPixel))
    cdef int i, x, y
    cdef cRegion* cregion
    for i in range(n_seed_points):
        cregion = grid_new_region(grid)
        cregion_insert_pixels_coords(cregion, grid.pixels,
...
cpdef list feature_split_regiongrow(
    ...
    if debug: log.debug("< feature_split_regiongrow")

    # Check uniqueness of feature ids
    fids = set([feature.id] + [f.id for f in seed_features])
    if len(fids) <= len(seed_features):
        err = "duplicate feature ids: {}".format(", ".join([str(fid)
                for fid in [feature.id] + [f.id for f in seed_features]]))
        raise ValueError(err)
    ...
    if debug: log.debug("  prepare lookup table for {} feature pixels".format(feature.n))
    cdef int i, j
    cdef cPixel* cpixel
    ...
    if debug: log.debug("  remove non-overlapping seed points")
    cdef cRegion* cregion
    ...
    # Create cregions for new subregions
    if debug: log.debug("  create cregions for new subregions")
    ...
    if sum([f.n for f in subfeatures]) != feature.n:
        err = ("splitting of {} failed: total number of pixels wrong: {} != {}"
                ).format(feature.id, "+".join([str(f.n) for f in subfeatures]),
                feature.n)
        raise Exception(err)

    # Clean up memory
    if debug: log.debug(" clean up memory")
    cregions_cleanup(&cregions_seeds, cleanup_regions=True)
    grid_cleanup(&grid)
    feature.reset_cregion(warn=False)
...
cpdef list features_grow(
    ...
    if len(fids) < len(features):
        err = "duplicate feature ids: {}".format(", ".join([str(fid)
                for fid in [f.id for f in features]]))
        raise ValueError(err)
...
cdef void cfeatures_grow_core(
        int       ngrow,
        cRegions* cregions_orig,
        cRegions* cregions_grown,
        cGrid*    grid,
    ) except *:
    cdef bint debug = False
    if debug: log.debug("< cfeatures_grow_core")
    cdef int n_regions = cregions_orig.n
    cdef int i, j, k, i_region, neighbor_rank
    cdef cRegion* cregion
    cdef cPixel* cpixel
    cdef np.int32_t x, y
    ...
    cdef np.ndarray[np.int32_t, ndim=2] grid_pixels = np.array(
            np.meshgrid(
                    np.arange(grid.constants.nx), np.arange(grid.constants.ny)
                ), np.int32).reshape([2, grid.constants.nx*grid.constants.ny]).T
    ...
    for i in range(n_regions):
        cregions_link_region(cregions_grown, grid_new_region(grid),
                cleanup=False, unlink_pixels=False)

    # Initialize pixel status table for feature
    pixel_status_table_init_feature(
            grid.pixel_status_table,
            &cfeature,
            cregions_orig
        )
    ...
    if debug: log.debug("  prepare seed pixels etc.")
    cdef int n, l
    ...
    for i in range(n_regions):
        ...
        cregions_link_region(&features_iteration_seeds, cregion_iter,
                cleanup=False, unlink_pixels=False)
        ...
        for j in range(n):
            cpixel = pixels_tmp[j]
            cregion_insert_pixel(cregion, cpixel, link_region=True,
                    unlink_pixel=False)
            cregion_insert_pixel(cregion_iter, cpixel, link_region=False,
                    unlink_pixel=False)
        free(pixels_tmp)
    ...
    if debug: log.debug("  grow regions")
    cdef iter_i, iter_max=ngrow
    for iter_i in range(iter_max):
        if debug: log.debug("\n  <<<<<<<<<< ITER {} <<<<<<<<<<<<".format(iter_i))

        # Grow region by one pixel along boundary
        regiongrow_advance_boundary(
                &features_iteration_seeds,
                cregions_grown,
                n_regions,
                grid,
            )
        if debug: assert_no_unambiguously_assigned_pixels(&cfeature, grid.pixel_status_table)
        ...
        for i in range(features_iteration_seeds.n):
            ...
            if debug: log.debug("  DONE in {} iterations (features_iteration_seeds is NULL)".format(iter_i+1))
            break
    ...
    for i in range(cregions_grown.n):
        pixel_status_table_reset_feature(grid.pixel_status_table,
                cregions_grown.regions[i])
...
cpdef void _replace_feature_associations(Feature feature,
    ...
    if debug: log.debug("{}: {} + [{}] => [{}]".format(_name_, feature.id, ", ".join([str(f.id) for f in seed_features]), ", ".join([str(f.id) for f in subfeatures])))

    cdef Feature seed_feature, subfeature, main_feature_assc
    cdef list main_associates, seed_associates
    cdef str name_main, name_assc
        ...
        for main_feature_assc_ in main_associates.copy():
            ...
            if isinstance(main_feature_assc_, Feature):
                ...
                for seed_feature in seed_features:
                    ...
                    if debug: log.debug("feature {}: remove {} associate {}".format(feature.id, name_assc, seed_feature.id))
                    ...
                    for name_main, seed_associates in (
                        ...
                        if debug: log.debug("feature {}: remove {} associate {}".format(seed_feature.id, name_main, feature.id))
                        ...
                        for subfeature in subfeatures:
                            if subfeature.overlaps(seed_feature):
                                if debug: log.debug("feature {}: add {} associate {}".format(seed_feature.id, name_main, subfeature.id))
                                seed_associates.append(subfeature)
                                if name_assc not in subfeature.associates:
                                    subfeature.associates[name_assc] = []
                                if debug: log.debug("feature {}: add {} associate {}".format(subfeature.id, name_assc, seed_feature.id))
                                subfeature.associates[name_assc
                                        ].append(seed_feature)
                                break
                        else:
...
def split_regiongrow_levels(
    ...
    if filter_mask is None:
        ...
        if filter_mask.shape != (nx, ny):
            err = "filter_mask: invalid shape: {} != ({}, {})".format(
                    filter_mask.shape, nx, ny)
            raise ValueError(err)
...
cdef list csplit_regiongrow_levels(
    ...
    cdef bint debug = False
    if debug: log.debug("< csplit_regiongrow_levels {}".format(levels))
    cdef int i, j, k, i_feature
        ...
        for i in range(subfeatures_tmp.n):
            ...
            if minsize > 1:
                ...
                if cregion.pixels_n < minsize:
                    if debug: log.debug("remove [{}] (n={}): too small (< {})".format(cregion.id, cregion.pixels_n, minsize))
                    continue
            if maxsize > 0:
                cregion = subfeatures_tmp.regions[i]
                if cregion.pixels_n > maxsize:
                    if debug: log.debug("remove [{}] (n={}): too big (> {})".format(cregion.id, cregion.pixels_n, maxsize))
                    continue
            ...
            free(pixels_tmp)
        if debug: log.debug("subfeatures_final.n={}".format(subfeatures_final.n))
    ...
    if debug: log.debug("> csplit_regiongrow_levels {}".format(len(subfeatures)))
    ...
    for i_level in range(n_levels):
        ...
        for i_region in range(subfeatures_tmp.n):
            ...
            if subfeatures_seeds.n > nmax_seeds[0]:
                pixel_region_table_grow(grid.pixel_region_table, cfeature,
                        subfeatures_seeds.n)
                nmax_seeds[0] = subfeatures_seeds.n

            # Split subfeature from seeds
            # print("[{}] SPLIT SUBFEATURE".format(i_region))
        ...
        for i in range(subfeatures_new.n):
            cregion = grid_new_region(grid)
            cregions_link_region(subfeatures_tmp, cregion, cleanup=False, unlink_pixels=False)
...
cdef void extract_subregions_level(
    ...
    cdef bint debug = False
    if debug: log.debug("< extract_subregions_level {} {}".format(cregion.id, level))
    ...
    cdef int i_pixel, i_pixel_start, i, j, k, n, i_neighbor
    cdef cPixel* cpixel
    ...
    for i_pixel_start in range(n):
        ...
        if valid_subregion:
            ...
            cregions_link_region(cregions_sub, cregion_sub, cleanup=False, unlink_pixels=False)
    ...
    if debug:
        ...
        for i in range(cregion.pixels_max):
            ...
            if cpixel.region is NULL:
                log.debug("[{}] ({}, {})".format(-1, cpixel.x, cpixel.y))
            else:
                log.debug("[{}] ({}, {})".format(cpixel.region.id, cpixel.x, cpixel.y))
        log.debug(cregions_sub.n, cregions_sub.regions[0].id, cregions_sub.regions[1].id)
    ...
    if debug:
        for i in range(cregions_sub.n):
            ...
            for j in range(cregions_sub.regions[i].pixels_max):
                if cregions_sub.regions[i].pixels[j] is not NULL:
                    log.debug("[{}] PIXEL ({}, {})".format(cregions_sub.regions[i].id, cregions_sub.regions[i].pixels[j].x, cregions_sub.regions[i].pixels[j].y))
...
cdef void collect_adjacent_pixels(
    ...
    for iter_i in range(iter_max):
        ...
        err = "identify_regions: loop timed out (nmax={})".format(iter_max)
        raise Exception(err)
    cregion_cleanup(
            &pixels_todo,
            unlink_pixels=False,
...
cdef void csplit_regiongrow_core(
    ...
    cdef bint debug = False
    if debug: log.debug("< csplit_regiongrow_core")
    cdef int i, j, k, i_region, neighbor_rank
    cdef cRegion* cregion
    cdef cPixel* cpixel
    cdef np.int32_t x, y
    ...
    for i in range(n_subregions):
        cregions_link_region(subfeature_cregions, grid_new_region(grid),
                cleanup=False, unlink_pixels=False)
    ...
    for i in range(n_subregions):
        ...
        cregion_iter = grid_new_region(grid)
        cregions_link_region(&subfeatures_iteration_seeds, cregion_iter,
                cleanup=False, unlink_pixels=False)
        n = cregion_seeds.pixels_n
        pixels_tmp = <cPixel**>malloc(n*sizeof(cPixel*))
        l = 0
    ...
    for iter_i in range(iter_max):
        ...
        for i in range(subfeatures_iteration_seeds.n):
            ...
            if debug: log.debug("DONE in {} iterations (subfeatures_iteration_seeds is NULL)".format(iter_i+1))
            break
...
cdef void regiongrow_advance_boundary(
    ...
    cdef bint debug = False, superdebug = False
    if debug: log.debug("< regiongrow_advance_boundary")
    cdef int i, j, k, i_region, i_pixel, i_neighbor, i_subregion, neighbor_rank
    cdef cRegion* cregion
    cdef cPixel* cpixel
    cdef cPixel* neighbor
    ...
    if debug:
        ...
        for i_region in range(subfeatures_iteration_seeds.n):
            ...
            log.debug("cregion {}: {}".format(cregion.id, cregion.pixels_n))
            if superdebug:
    ...
    for i_region in range(subfeatures_iteration_seeds.n):
        ...
        for i_pixel in range(cregion_seeds.pixels_n):
            ...
            if superdebug: log.debug(" [{}] {} ({}, {})".format(i_region, i_pixel, cpixel.x, cpixel.y))
            ...
            for i_neighbor in range(cpixel.neighbors_max):
                ...
                with cython.cdivision(True):
                    if i_neighbor%2 == 0:
                        ...
                        neighbor_rank = 1
                if superdebug: log.debug(" -> neighbor {}: ({}, {}) ({})".format(i_neighbor, neighbor.x, neighbor.y, neighbor_rank))
                ...
                if newly_assigned:
                    if debug: log.debug("newly assigned ({}, {})".format(neighbor.x, neighbor.y))
                    cregion_insert_pixel(
                            &newly_assigned_pixels,
                            neighbor,
                            link_region=False,
    ...
    for i_pixel in range(newly_assigned_pixels.pixels_istart,
            newly_assigned_pixels.pixels_iend):
        cpixel = newly_assigned_pixels.pixels[i_pixel]
        if cpixel is NULL:
            continue
...
cdef void regiongrow_resolve_multi_assignments(
    ...
    for i_pixel in range(cregion_multi_assigned.pixels_istart,
                cregion_multi_assigned.pixels_iend):
        ...
        if debug: log.debug("({}, {}): {} regions".format(cpixel.x, cpixel.y, grid.pixel_region_table[cpixel.x][cpixel.y].n))
        ...
        if debug: log.debug(" -> select region {}".format(selected_region_ranked.region.id))
        pixel_region_table_reset_slots(
                grid.pixel_region_table,
                cpixel.x,
                cpixel.y,
            )
        if debug: log.debug("pixel_region_table_insert_region: ({}, {}) [{}] {}".format(x, y, selected_region_ranked.region.id, selected_region_ranked.rank))
...
cdef cRegionRankSlot* resolve_multi_assignment(
    ...
    if debug: print_selected_regions_dbg(1, selected_regions)
    ...
    if debug: print_selected_regions_dbg(2, selected_regions)
    if selected_regions.n == 1:
        return &selected_regions.slots[0]
    ...
    err = ("pixel ({}, {}) could not be assigned unambiguously "
...
cdef void print_selected_regions_dbg(
    ...
    if selected_regions.n == 1:
        ...
        log.info("[{}] found best region: {}".format(i_test, cregion.id))
...
cdef void resolve_multi_assignment_best_connected_region(
    ...
    """
    cdef bint debug = False
    if debug: log.debug("< resolve_multi_assignment_best_connected_region ({}, {})".format(cpixel.x, cpixel.y))
    cdef int i, i_slot, i_region
    cdef np.int32_t x=cpixel.x, y=cpixel.y
    cdef int n_direct_neighbors, n_indirect_neighbors, neighbor_rank
    cdef int n_direct_neighbors_max=0, n_indirect_neighbors_max=0
    for i_slot in range(grid.pixel_region_table[x][y].max):
        ...
        if n_direct_neighbors > 0:
            ...
            neighbor_rank = 1
        if debug: log.debug(" [{}] {} direct, {} indirect".format(cregion.id, n_direct_neighbors, n_indirect_neighbors))

        # Check if the region beats the previous leader
        if (n_direct_neighbors < n_direct_neighbors_max or
                (n_direct_neighbors == n_direct_neighbors_max and
                    n_indirect_neighbors < n_indirect_neighbors_max)):
            # Nope; discard this region
            continue
        elif (n_direct_neighbors > n_direct_neighbors_max or
                n_indirect_neighbors > n_indirect_neighbors_max):
...
cdef void cpixel_count_neighbors_in_cregion(
    ...
    cdef bint debug = False
    if debug: log.debug("< cpixel_count_neighbors_in_cregion: ({}, {})[{}] [{}]".format(cpixel.x, cpixel.y, "-" if cpixel.region is NULL else cpixel.region.id, cregion.id))
    cdef int i_neighbor
    ...
    for i_neighbor in range(grid.constants.n_neighbors_max):
        ...
        neighbor = cpixel_get_neighbor(cpixel, i_neighbor, grid.pixels,
                grid.constants.nx, grid.constants.ny, connectivity=8)
        if neighbor is NULL:
            continue
        with cython.cdivision(True):
            if i_neighbor%2 == 0:
        ...
        if debug: log.debug("neighbor {}: ({}, {})[{}]".format(i_neighbor, neighbor.x, neighbor.y, "-" if neighbor.region is NULL else neighbor.region.id))
        ...
        if -1 <= status <= 0:
            if direct_neighbor:
                ...
                if debug: log.debug(" => direct neighbor no. {}".format(n_direct_neighbors[0]))
            else:
...
cdef bint regiongrow_assign_pixel(
    ...
    # Pixel has not yet been assigned
        ...
        err = ("error: ({}, {}): invalid pixel status {}"
                ).format(x, y, grid.pixel_status_table[x][y])
        raise Exception(err)
...
cpdef find_minima_2d(fld, n=4, nmax_extrema=100):
    if n not in [4, 8, 12, 20]:
        err = "Invalid parameter n={}".format(n)
        raise ValueError(err)
...
cdef int _c_find_extrema_2d_core(
    ...
    cdef int i, j, nxs, nxe, nys, nye, n_extrema=0
    cdef np.int32_t nx=fld.shape[0], ny=fld.shape[1]
    cdef np.float32_t v
    if n == 4 or n == 8:
        nxs = 1
        nys = 1
    ...
    for i in range(nxs, nxe):
        for j in range(nys, nye):
            v = fld[i, j]
            if n >= 4:
                if (sign*v <= sign*fld[i-1, j  ] or
                    sign*v <= sign*fld[i  , j+1] or
                    sign*v <= sign*fld[i+1, j  ] or
                    sign*v <= sign*fld[i  , j-1]):
                    continue
            if n >= 8:
                if (sign*v <= sign*fld[i-1, j+1] or
                    sign*v <= sign*fld[i+1, j+1] or
                    sign*v <= sign*fld[i+1, j-1] or
                    sign*v <= sign*fld[i-1, j-1]):
                    continue
            if n >= 12:
                if (sign*v <= sign*fld[i-2, j  ] or
                    sign*v <= sign*fld[i  , j+2] or
                    sign*v <= sign*fld[i+2, j  ] or
                    sign*v <= sign*fld[i  , j-2]):
                    continue
            if n >= 20:
                if (sign*v <= sign*fld[i-2, j-1] or
                    sign*v <= sign*fld[i-2, j+1] or
                    sign*v <= sign*fld[i-1, j+2] or
                    sign*v <= sign*fld[i+1, j+2] or
                    sign*v <= sign*fld[i+2, j+1] or
                    sign*v <= sign*fld[i+2, j-1] or
                    v >= fld[i+1, j-2] or
...
cdef void features_to_cregions(
    ...
    cdef bint debug = False
    if debug: log.debug("< {}: {} features".format(_name_, len(features)))
    if debug: log.debug("{}".format(pformat(features)))
    cdef int i, j
    cdef Feature feature
    cdef cRegion* cregion
    ...
    for i in range(n_regions):
        feature = features[i]
        if debug: log.debug("convert feature {}/{}: [{}/]({}/)".format(
                i+1, n_regions, feature.id, feature.n))
        ...
        if feature.cregion is not NULL:
            msg = "warning: {}: feature {}: cregion not NULL: ".format(
                    _name_, feature.id)
            if reuse_feature_cregion:
                log.warning(msg+"reuse existing")
                continue
            else:
                log.warning(msg+"clean up existing")
        ...
        if debug: log.debug(" -> feature({}) to cregion({}) ({} pixels)".format(feature.id, feature.cregion.id, feature.cregion.pixels_n))
    free(fids)

    # Add neighbors (i.e. connected regions)
    if debug: log.debug("add neighbors (i.e. connected regions)")
    features_neighbors_to_cregions_connected(features, cregions,
            ignore_missing_neighbors)
        ...
        for feature1 in features:
            ...
            for feature2 in features:
                if (feature1 == feature2 or
                        (feature2.id, feature1.id) in shared_fids):
                    continue
                pxs2 = set([(x, y) for x, y in feature2.pixels])
                shared = sorted(pxs1.intersection(pxs2))
                if len(shared) > 0:
                    shared_fids[(feature1.id, feature2.id)] = shared
        err_core = "{}".format(len(shared_fids),
                "\n".join([" [{}]<->[{}]: {}".format(fid1, fid2,
                        ", ".join(["({}, {})".format(x, y) for x, y in pxs]))
...
cdef void features_neighbors_to_cregions_connected(
    ...
    cdef int i_region, n_regions=len(features_list), n_neighbors
    cdef Feature feature, feature_neighbor, feature_neighbor_2
    cdef cRegion* cregion
    cdef cRegion* cregion_neighbor
    if not n_regions == cregions.n:
        err = "{}: numbers of features ({}) and cregions ({}) differ".format(
                _name_, n_regions, cregions.n)
        raise Exception(err)
    cdef int i_region_2
    for i_region in range(cregions.n):
        feature = features_list[i_region]
        n_neighbors  = len(feature.neighbors)
        if debug: log.debug("feature {}: {} neighbors".format(feature.id, n_neighbors))
        if n_neighbors == 0:
            continue
        ...
        for i_neighbor in range(n_neighbors):
            ...
            if feature_neighbor.id == feature.id:
                err = ("feature {} cannot be it's own neighbor!"
                        ).format(feature.id)
                raise Exception(err)
            ...
            for i_region_2 in range(cregions.n):
                ...
                if not ignore_missing_neighbors:
                    err = "feature {}: missing neighbor {}".format(feature.id,
                            feature_neighbor.id)
                    raise Exception(err)
                continue
...
cdef void feature_to_cregion(
    ...
    for i in range(n_pixels):
        ...
        cregion_insert_pixel(
                ...
                &grid.pixels[x][y],
                link_region=True,
                # necessary (otherwise 'shell pixel connected to wrong region')
                unlink_pixel=True,
            )
...
cdef class Pixel:
    ...
    def __init__(Pixel self, int x, int y, float v,
            Field2D fld, np.uint64_t id):
        self.id = id
        self.x = x
        self.y = y
        self.v = v
    ...
    def __repr__(self):
        return "{c}({x}, {y}, {v}, {t})".format(c=self.__class__.__name__,
                x=self.x, y=self.y, v=self.v, t=self.type)
...
cdef class Feature:
    ...
    cpdef void set_center(self, np.ndarray[np.int32_t, ndim=1] center,
            bint derive=True) except *:
        if center is None:
            if derive:
                if self.pixels is None:
    ...
    cdef void set_cregion(self, cRegion* cregion) except *:
        if self.debug: print("{}.set_cregion({})".format(self, "NULL" if cregion is NULL else cregion.id))
    ...
    cpdef void reset_cregion(self, bint warn=True) except *:
        ...
        if warn:
            log.warning("Feature2d.reset_cregion: might cause memory leaks "
    ...
    cpdef void cleanup_cregion(self, unlink_pixels=True, reset_connected=True) except *:
        ...
        if self.debug: print("{}.cleanup_cregion({})".format(self, "NULL" if self.cregion is NULL else self.cregion.id))
        if self.cregion is not NULL:
    ...
    def _check_pixels_present(self, action):
        ...
        if self.pixels is None:
            err = "cannot {} without pixels (None)".format(action)
            raise Exception(err)

        if self.pixels.size == 0:
            err = "cannot {} without pixels (size == 0)".format(action)
            raise Exception(err)

    def derive_center_from_pixels(self):
        self._check_pixels_present("derive center")
        raise NotImplementedError("Feature.derive_center_from_pixels()")
    ...
    def area_lonlat(self, lon, lat, *, unit="km2", method="grid"):
        ...
        if lon is None:
            raise ValueError("lon is None (method='{}')".format(method))
        if lat is None:
            raise ValueError("lat is None (method='{}')".format(method))

        # Check units
        units = ["m2", "km2"]
        if unit not in units:
            err = "invalid unit '{}', must be one of {}".format(unit, units)
            raise ValueError(err)
        ...
        if method == "grid":
            ...
            if len(lon.shape) > 1:
                err = ("{}.area_lonlat with method 'grid' for 2D lon/lat; "
                        "if your lon/lat grid is regular, pass 1D lon/lat, "
    ...
    def area_lonlat_proj(self, lon, lat):
        ...
        if len(self.holes) > 0:
            log.warning(("feature {}: area_lonlat: subtracting area of {} "
                    ...
                    "is slightly overestimated (include the holes' boundary "
    ...
    def from_jdat(cls, jdat, name=None, pixel_tables=None,
        ...
        _name_ = "{}.from_jdat".format(cls.__name__)
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef np.ndarray[np.int32_t, ndim=2] overlap_bbox(
            self, Feature other):
        cdef np.int32_t x0min = self.pixels[:, 0].min()
        cdef np.int32_t x1min = other.pixels[:, 0].min()
        cdef np.int32_t x0max = self.pixels[:, 0].max()
    ...
    cdef int _overlap_n_core(self,
        ...
        for i in prange(n0, nogil=True):
            x0 = pixels0[i, 0]
            y0 = pixels0[i, 1]
            if (x0 < bbox[0, 0] or x0 > bbox[1, 0] or
                    y0 < bbox[0, 1] or y0 > bbox[1, 1]):
                continue
            for j in range(n1):
                x1 = pixels1[j, 0]
                y1 = pixels1[j, 1]
    ...
    def shared_boundary_pixels(self, key, mode="unique"):
        if mode == "unique":
            ...
            err = "invalid mode '{}' (must be one of {})".format(
                    mode, ", ".join(["unique", "complete"]))
            raise ValueError(err)
    ...
    cpdef np.ndarray to_field(self, int nx, int ny, np.float32_t bg=0,
            bint fill_holes=False, bint cache=False):
        ...
        cdef int i, n=len(self.pixels), x, y
        cdef np.float32_t v
        for i in range(n):
            x = self.pixels[i, 0]
            y = self.pixels[i, 1]
    ...
    cpdef np.ndarray to_mask(self, int nx, int ny, bint fill_holes=False,
            object type_=np.bool, bint cache=False):
    ...
    cpdef object vertex(self):
        if self._vertex_name is not None:
            try:
                ...
                err = "error retrieving vertex '{}': {}".format(
                        self._vertex_name, e)
                raise Exception(err)
...
cdef list cregions_create_features(
    ...
    cdef np.ndarray[np.int32_t, ndim=2] shell
    cdef np.ndarray[np.int32_t, ndim=2] hole
    cdef list features = [], shells, holes
...
cdef void cregions2features_connected2neighbors(
    ...
    cdef Feature feature, other_feature
    cdef dict source, target

    cdef int n_regions_max = 3
    pixel_region_table_reset(grid.pixel_region_table, grid.constants.nx, grid.constants.ny)

    cdef cRegion cregion_bg
    cregion_init(&cregion_bg, cregion_conf_default(), cregion_get_unique_id())
    initialize_surrounding_background_region(cregions, &cregion_bg, grid,
            link_region=True)
    ...
    for i_cregion in range(cregions.n):
        ...
        for i_cregion in range(cregion.connected_n):
            ...
            target[other_feature] = np.array(sorted(
                    source[other_cregion.id]), dtype=np.int32)
...
cdef void determine_shared_boundary_pixels(
    ...
    cdef bint debug = False
    if debug: log.debug("< determine_shared_boundary_pixels [{}]".format(cregion.id))
    cdef int i, j, i_region
    cdef cRegion* other_cregion
    cdef tuple pixel
    ...
    for i in range(boundary_pixels.pixels_istart,
        ...
        for j in range(grid.constants.n_neighbors_max):
            ...
            if cpixel.neighbors[j] is NULL:
                continue
            if debug: log.debug("({}, {}).neighbor[{}]: ({}, {})".format(cpixel.x, cpixel.y, j, cpixel.neighbors[j].x, cpixel.neighbors[j].y))
            if cpixel.neighbors[j].region is NULL:
                interior = False
                if debug: log.debug(" -> background")
                pixel_region_table_insert_region(
                        grid.pixel_region_table,
                        cpixel.x,
                        cpixel.y,
                ...
                if debug: log.debug(" -> region {}".format(other_cregion.id))
                pixel_region_table_insert_region(
                        grid.pixel_region_table,
                        cpixel.x,
                        cpixel.y,
            ...
            if debug:
                ...
                for i in range(grid.pixel_region_table[x][y].n):
                    if grid.pixel_region_table[x][y].slots[i].region.id == cregion_bg.id:
                        log.debug("  - region {} (background)".format(grid.pixel_region_table[x][y].slots[i].region.id))
    ...
    for i_pixel in range(boundary_pixels.pixels_istart,
        ...
        for i_slot in range(grid.pixel_region_table[x][y].n):
            ...
            if cregion_assoc is NULL:
                ...
                if debug: log.debug("SHARED[in] <- ({}, {})".format(*xy))
    ...
    for i_pixel in range(boundary_pixels.pixels_istart,
        ...
        if cregion_assoc is NULL:
            ...
            if debug: log.debug("UNIQUE[in] <- ({}, {})".format(*xy))
...
cdef void initialize_surrounding_background_region(
    ...
    cdef int mode = 1
    cdef int i_region, i_pixel, i_shell, i_hole
    cdef int x, y
    if mode == 0:
        ...
        err = ("initialize_surrounging_background_region: invalid mode {}"
                ).format(mode)
        raise Exception(err)
...
cdef Feature cregion_find_corresponding_feature(
    ...
    for i in range(n_features):
        ...
        err = "error: cregion id {} not found in table".format(cregion.id)
        raise Exception(err)
    for i in range(n_features):
        feature = features[i]
        if feature.id == fid:
        ...
        for neighbor in feature.neighbors:
            if isinstance(neighbor, Feature):
                ...
                if missing_action == "ignore":
                    continue
                msg = "{}: feature {}: missing neighbor {}".format(
                        _name_, feature.id, neighbor)
...
cpdef void associate_features(
    ...
    cdef Feature feature, feature1, feature2
    cdef int i, i1, i2, n1=len(features1), n2=len(features2)
    ...
    if check_timesteps:
        ...
        if check_timesteps:
            for i in range(n1):
                feature = features1[i]
                if feature.timestep != timestep:
                    err = "inconsistent timesteps: {} != {}".format(
                            feature.timestep, timestep)
                    raise Exception(err)
            for i in range(n2):
                feature = features2[i]
...
cpdef int resolve_indirect_associations(
    ...
    cdef int i, n, n_fids2, n_fids3
    cdef int i_feature1, n_features1=len(features1)
    cdef int i_feature2, n_features2=len(features2)
    cdef int i_feature3, n_features3=len(features3)
    cdef Feature feature1, feature2, feature3
    cdef dict associates1, associates2, associates3
    cdef list fids2, fids3
    cdef np.uint64_t fid1, fid2, fid3
    cdef str assoc_name13, assoc_name_31
    cdef list assoc_ids_13, assoc_ids_31
    cdef list ids
            ...
            for feature in features:
                for assc_name, assc_features in feature.associates.items():
                    feature.associates[assc_name] = [
                            (f.id if isinstance(f, Feature) else f)
                            for f in assc_features]
    else:
        if names is not None:
            err = ("argument names meaninless unless features are passed "
                    "in named form in a dict")
...
def features_associates_id2obj(features_named, names=None,
        missing_ids=None, missing_action="error", missing_verbose=True):
    ...
    if missing_action not in ["error", "warning", "ignore"]:
        raise ValueError("invalid missing_action '{}'".format(
                missing_action))
            ...
            for assoc_name, fids in feature.associates.items():
                ...
                for fid in fids.copy():
                    try:
                        ...
                        err = "associated {} feature not found: {} ".format(
                                assoc_name, fid)
    ...
    if len(missing_ids) > 0:
        if missing_action == "warning":
            if len(missing_ids) == 1:
                ...
                log.warning("{} associated {} features not found".format(
                        name, len(fids)))
            else:
                n = sum([len(i) for i in missing_ids.values()])
                log.warning("{} associated features not found: {}".format(n,
                        ", ".join(["{}*{}".format(len(ids), name) for name, ids
...
def oldfeature_to_pixels(oldfeature, lon, lat, vb=True):
    ...
    extrema = np.array([_get_indices(pt.xy, kdtree, lon.shape)
            for pt in oldfeature.minima()])
    shell = np.array([_get_indices(xy, kdtree, lon.shape)
...
def cyclones_to_features(ts, cyclones, slp, lon, lat, vb=True, out=None):
    ...
    for cyclone in cyclones:
        if vb: print("convert cyclone {}".format(cyclone.id()))

        # Extract feature pixels
        pixels, extrema, center, shell = oldfeature_to_pixels(cyclone, lon, lat)
        if vb: print("extracted {} pixels, {} extrema, and center".format(
                len(pixels), len(extrema)))

        # Turn pixels into new-style features
        if vb: print("create Feature object")
