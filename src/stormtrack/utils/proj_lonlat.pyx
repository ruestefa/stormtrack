...
def circle_lonlat_mask(clon, clat, rad_km, dlon, dlat, method="dyntools",
    ...
    if method == "pyproj":
        if abs(clat) > 45:
            print(("warning: circle_lonlat_mask: abs(clat) {} > 45: "
...
def circle_lonlat_mask_dyntools(clon, clat, rad_km, dlon, dlat,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cmd = ("module load dyn_tools && gridding {cf} {of} {r} "
                "{lo0} {lo1} {la0} {la1} {dlo} {dla}").format(**kwas)
        out = subprocess.check_output(["/bin/bash", "-c", cmd])
        out = out.decode("utf-8").strip()
        try:
            os.rename(fld_file+".cdf", fld_file)
        except FileNotFoundError:
            err = ("ERROR gridding circle on globe!\n\ncommand:\n{}: "
                    "\n\noutput:\n{}").format(cmd, out)
