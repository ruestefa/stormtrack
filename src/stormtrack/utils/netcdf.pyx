...
def timestep2timestamp(timestep, fmt=None):
    ...
    if isinstance(timestep, str):
        ...
        if fmt is None:
            try:
                ...
                err = ("no default format for {}-digit timestep among {}"
                        ).format(len(timestep), sorted(fmts_default))
                raise Exception(err)
    ...
    try:
        ...
        err = ("cannot derive timestamp from timestep; "
