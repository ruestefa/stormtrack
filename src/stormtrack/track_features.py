...
def main(conf_in, conf_out, conf_tracker, conf_track_pp, conf_exe,
        conf_preproc, conf_topo, conf_idfy, conf_split):
    ...
    if conf_exe["timings_measure"]:
        with timings["input"].get_lock():
            timings["input"].value += timer() - timings.pop(
                    timkey("input_start"))
    ...
    if conf_tracker["size_lonlat"]:
        ...
        if lon1d is None or lat1d is None:
            err = ("cannot compute Feature.area_lonlat (--size-lonlat) on "
    ...
    for timesteps_out, outfile in outfiles_tss_sorted:
        ...
        for timesteps_in, infile in infiles_tss_sorted:
            if all(ts in timesteps_out for ts in timesteps_in):
                ...
                err = ("input period ({}..{}) only partly overlaps output "
                        "period ({}..{}); must be even subperiods!").format(
                        min(timesteps_in), max(timesteps_in),
                        min(timesteps_out), max(timesteps_out))
                raise Exception(err)
            else:
                continue
    ...
    for timesteps_out, _outfile_next in outfiles_tss_sorted:
        ...
        if not conf_exe["overlay_identification_tracking"]:
            ...
            for timesteps_in, infile in infiles_tss_sorted_i:
                ...
                run_feature_input(features_ts, infos_features, grid,
                        lon2d, lat2d, conf_in, conf_out, conf_topo,
                        conf_preproc, conf_idfy, conf_split, conf_exe,
                        timesteps_in, infile,
                    )

                run_tracking_timesteps(tracker, finished, pending, features_ts,
                        infos_features, info_tracks, outfiles, nts,
                        lon2d, lat2d, grid, conf_in, conf_out, conf_topo,
                        conf_preproc, conf_idfy, conf_split, conf_track_pp,
                        conf_exe, its_done, timestep_prev, timesteps_in
                    )
            ...
            args_id = (infos_features, grid, lon2d, lat2d, conf_in, conf_out,
                    conf_topo, conf_preproc, conf_idfy, conf_split, conf_exe,
                    features_ts, infiles_tss_sorted_i, conf_exe["num_procs_id"])
            p_id = mp.Process(target=fct_id, args=args_id)
            p_id.start()

            # Track features
            _run_feature_tracking__tss(tracker, finished, pending, features_ts,
                    infos_features, info_tracks, outfiles, nts, lon2d, lat2d,
                    grid, conf_in, conf_out, conf_topo, conf_preproc, conf_idfy,
                    conf_split, conf_track_pp, conf_exe, its_done,
                    timestep_prev, infiles_tss_sorted_i)
    ...
    if conf_exe["timings_measure"]:
        with timings["core"].get_lock():
            timings["core"].value += timer() - timings.pop(
                    timkey("core_start"))

    # Write remaining tracks to disk
    if finished:
        pending_i = dict(
                ts   = timestep_prev.value,
                file = outfiles["curr"],
                iout = conf_out["iout"],
            )
        ...
        for pending_i in pending:
            nx, ny = lon2d.shape
            write_pending_tracks(finished, infos_features["curr"], info_tracks,
                    nx, ny, conf_out, timestep, pending_i)
        if conf_exe["timings_measure"]:
            with timings["output"].get_lock():
                timings["output"].value += timer() - timings.pop(
                        timkey("output_start"))
...
def _run_feature_input__tss(infos_features, grid, lon2d, lat2d, conf_in,
        conf_out, conf_topo, conf_preproc, conf_idfy, conf_split, conf_exe,
        features_ts, infiles_tss_sorted_i, num_procs_id
    ):
    fct = functools.partial(_run_feature_input__tss__seq, infos_features,
            grid, lon2d, lat2d, conf_in, conf_out, conf_topo, conf_preproc,
            conf_idfy, conf_split, conf_exe, features_ts)
...
def _run_feature_input__tss__seq(infos_features, grid, lon2d, lat2d, conf_in,
        conf_out, conf_topo, conf_preproc, conf_idfy, conf_split, conf_exe,
        features_ts, infiles_tss_sorted_i
    ):
    #if grid is None:
    #    nx, ny = lon2d.shape
    #    const = Constants.default(nx=nx, ny=ny)
    ...
    for timesteps_i, infile in infiles_tss_sorted_i:
        run_feature_input(features_ts, infos_features, grid, lon2d, lat2d,
            conf_in, conf_out, conf_topo, conf_preproc, conf_idfy, conf_split,
            conf_exe, timesteps_i, infile)

def _run_feature_tracking__tss(tracker, finished, pending, features_ts,
        infos_features, info_tracks, outfiles, nts,
        lon2d, lat2d, grid, conf_in, conf_out, conf_topo,
        conf_preproc, conf_idfy, conf_split, conf_track_pp,
        conf_exe, its_done, timestep_prev, infiles_tss_sorted_i
    ):
    #if grid is None:
    #    nx, ny = lon2d.shape
    #    const = Constants.default(nx=nx, ny=ny)
...
def run_tracking_timesteps(tracker, finished, pending, features_ts,
        infos_features, info_tracks, outfiles, nts, lon2d, lat2d, grid,
        conf_in, conf_out, conf_topo, conf_preproc, conf_idfy, conf_split,
        conf_track_pp, conf_exe, its_done, timestep_prev, timesteps_in
    ):

    for its_curr, timestep in enumerate(timesteps_in):
        its = its_done.value + its_curr

        run_tracking_timestep(tracker, finished, pending, features_ts,
                infos_features, info_tracks, outfiles, its, nts,
                lon2d, lat2d, grid, conf_in, conf_out, conf_topo,
                conf_preproc, conf_idfy, conf_split, conf_track_pp,
                conf_exe, timestep, timestep_prev
            )
...
def run_tracking_timestep(tracker, finished, pending, features_ts,
        infos_features, info_tracks, outfiles, its, nts, lon2d, lat2d, grid,
        conf_in, conf_out, conf_topo, conf_preproc, conf_idfy,
        conf_split, conf_track_pp, conf_exe, timestep, timestep_prev
    ):
    ...
    msg = "[{}-{}] {:4} finished, {:4} active".format(
            (ts0 if ts0 is not None else timestep), timestep,
            len(finished), len(tracker.active_tracks))
    if pending:
        msg += "; {} pending output".format(len(pending))
        if len(pending) == 1:
            msg += ": {}".format(pending[0]["ts"])
        else:
            msg += "s: {}{}{}".format(pending[0]["ts"],
    ...
    if outfiles["next"] != outfiles["curr"]:
        ...
        _pending_i = dict(ts=timestep_prev.value, file=outfiles["curr"],
                iout=conf_out["iout"])
    ...
    for pending_i in [p for p in pending]:
        if (min_ts_start is not None and
                min_ts_start <= pending_i["ts"]):
            break
        pending.remove(pending_i)
        nx, ny = lon2d.shape
        write_pending_tracks(finished, info_features, info_tracks,
                nx, ny, conf_out, timestep, pending_i)
    ...
    for iter_i in range(iter_max):
        ...
        err = ("TIMEOUT ({:,} s) waiting for features at timestep {}\n"
    ...
    if conf_exe["timings_measure"]:
        with timings["wait"].get_lock():
            timings["wait"].value += timer() - timings.pop(
                    timkey("wait_start"))
    ...
    if infos["prev"] is not None:
        if infos["prev"] != infos["curr"]:
            err = ("info dicts differ:\n\nNEW:\n{}\n\nOLD:\n{}\n"
                    ).format(pformat(sorted(infos["curr"].items())),
...
def restart_tracker(tracker, infiles_tss, outfiles_tss, feature_name,
        finished, pending, link_features, linked_files_tss):
    ...
    _r = read_feature_file(infile, feature_name=feature_name, counter=True,
    ...
    for tss, infile in infiles_tss_sorted[1:]:
        ...
        if link_features:
            linked_files_tss[outfile] = (
                    get_linked_feature_files(infile))
        iout += 1
        if ts_start in tss:
            break
    else:
        err = "restart timestep period not long enough: {} not found".format(
                ts_start)
        raise Exception(err)

    # Read remaining track files
    print("read remaining {} restart files".format(len(infiles_todo)))
    _r = read_feature_files(infiles_todo, feature_name=feature_name,
            extra_tracks=tracks, ignore_missing_neighbors=True,
            is_subperiod=True, counter=True, counter_core=True,
        )
    tracks = _f["tracks"]
...
def run_feature_input(features_ts, infos_features, grid, lon2d, lat2d,
        conf_in, conf_out, conf_topo, conf_preproc, conf_idfy, conf_split,
        conf_exe, timesteps_in, infile
    ):
...
def import_features(infile, timesteps, conf_in):
    ...
    _r = read_feature_file(infile, feature_name=feature_name,
            minsize=minsize, maxsize=maxsize, ignore_missing_neighbors=True,
            silent=~debug,
        )
...
def write_pending_tracks(finished, info_features, info_tracks, nx, ny,
        conf_out, timestep, pending):
...
def write_tracks(outfile, *, finished, info_features, info_tracks, nx, ny,
        timestep, iout, conf_out):

    log.info("+"*60)
    if (conf_out["skip_output"] or
            iout < conf_out["output_skip_start"] or
            iout >= (conf_out["nout"] - conf_out["output_skip_end"])):
        # Skip output for this step; dismiss the respective tracks
        log.info("skip output for timestep {}".format(timestep))

    else:
        # Write output to disk
        log.info("save {} tracks ({} features) to {}".format(
...
def track_features(tracker, features, timestep, finished, conf_track_pp):
    ...
    nold = len(tracks)
    if debug: log.debug("finished {} tracks".format(nold))
...
def split_active_tracks(tracker, finished, timestep, conf_track_pp):
    ...
    tracks = tracker.pop_finished_tracks()
    log.info("[{}-{}] active track splitting: finish {} tracks".format(
...
def finish_tracks(tracker, finished, conf_track_pp):
    ...
    tracks = tracker.pop_finished_tracks()
    if debug: log.debug("finished {} tracks".format(len(tracks)))
...
def postproc_tracks(tracks, tracker, conf_track_pp):
    ...
    if debug: log.debug("remove short tracks: {} -> {}".format(nold, len(tracks)))
...
def parser_add_group__in(parser):
    ...
    group.add_argument(
            "-i", "--infile-fmt",
            help    = ("template of input file names used to extract the "
    ...
    group.add_argument(
            "--restart-file-fmt",
            help    = ("track file (.json) from which to restart tracking "
                        "(with {YYYY} etc.)"),
            metavar = "file-fmt",
    ...
    group.add_argument(
            ...
            help    = ("restart timestep range (format: e.g., YYYYMMDDHH); "
...
def parser_add_group__out(parser):
    ...
    group.add_argument(
            "--link-features",
            help    = ("link to existing feature file to avoid writing then "
    ...
    group.add_argument(
            ...
            help    = ("when computing features on-the-fly, don't write "
    ...
    group.add_argument(
            ...
            help    = ("file format for existing feature files "
...
def parser_add_group__tracker(parser):
    ...
    group.add_argument(
            "--split-tracks",
            help    = ("parameter N to split tracks at branchings (-1: no "
...
def parser_add_group__exe(parser):
    ...
    group.add_argument(
            "-p", "--profile",
            help    = ("run cProfile; optionally pass the number of lines "
    ...
    group.add_argument(
            "--overlay-id-track",
            help    = ("overlay identification and tracking: run both in "
    ...
    group.add_argument(
            "--num-procs-id",
            help    = ("parallelize identification; only if identification "
...
def preproc_args(parser, kwas):
    ...
    for key in ["in", "out", "tracker", "track_pp", "exe",
            "preproc", "topo", "idfy", "split"]:
...
def preproc_args__in(parser, kwas, conf):
    ...
    conf["infiles_tss"] = TimestepStringFormatter(infile_fmt
            ).run(conf["timesteps"])
    ...
    for key in ["{YYYY}", "{MM}", "{DD}", "{HH}", "{NN}"]:
        if key in outfile_fmt and key not in infile_fmt:
            parser.error(("inconsistent input and output files: "
                    "{}, {}").format(infile_fmt, outfile_fmt))

    conf["identify_features"] = (not any(infile_fmt.endswith(suffix)
    ...
    if restart_ts_range and restart_file_fmt:
        ...
        try:
            ...
            parser.error(e)
        conf["restart_files_in_tss"] = TimestepStringFormatter(
                restart_file_fmt).run(restart_tss)
        conf["restart_files_out_tss"] = TimestepStringFormatter(
                kwas["conf_out"]["outfile_fmt"]).run(restart_tss)
    elif not restart_ts_range and not restart_file_fmt:
        conf["restart"] = False
        conf["restart_files_tss"] = None
    else:
        parser.error("must pass both --restart-file-fmt and "
...
def preproc_args__out(parser, kwas, conf):
    ...
    outfiles_tss = TimestepStringFormatter(conf.pop("outfile_fmt")
    ...
    if not conf["link_existing_feature_files"]:
        ...
        if conf["link_features"] and kwas["conf_in"]["identify_features"]:
            parser.error("incompatible options --link-features and "
        ...
        if not kwas["conf_in"]["identify_features"]:
            parser.error("incompatible options --link-existing-feature-files "
    ...
    if conf["link_existing_feature_files"]:
        linkable_files_tss = TimestepStringFormatter(
                conf["existing_feature_file_fmt"]).run(
...
def preproc_args__tracker(parser, kwas, conf):
    ...
    conf["reshrink_tracked_features"] = (
            kwas["conf_idfy"]["grow_features_n"] > 0)
