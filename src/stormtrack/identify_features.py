...
try:
    ...
    print("warning: fronts-related import failed: {}; cannot identify fronts!".format(e))
#SR_TMP>
...
try:
    ...
    print("warning: cyclones-related import failed: {}; cannot identify cyclones!".format(e))
#SR_TMP>

#==============================================================================

def main(*, conf_in, conf_preproc, conf_out, conf_topo, conf_idfy,
            conf_split, conf_exe):
    ...
    infiles_per_outfile = relate_files_ts(conf_out["outfiles_ts"],
            conf_in["infiles_tss"])

    # For each output file, read all necessary fields,
    # identify all features, and write them to disk
    if conf_exe["parallelize"]:
        read_identify_write_par(conf_in, conf_out, conf_topo,
                conf_preproc, conf_idfy, conf_split, conf_exe,
                lon, lat, infiles_per_outfile)
    else:
        read_identify_write_seq(conf_in, conf_out, conf_topo,
                conf_preproc, conf_idfy, conf_split, conf_exe,
                lon, lat, infiles_per_outfile)
...
def read_topo(conf_topo, conf_in):
    ...
    if reduce_stride > 1:
        conf_topo["fld"] = reduce_grid_resolution(conf_topo["fld"],
                reduce_stride, reduce_mode)

#==============================================================================

def read_identify_write_seq(conf_in, conf_out, conf_topo, conf_preproc,
        conf_idfy, conf_split, conf_exe, lon, lat, infiles_per_outfile):

    for timesteps_out__outfile in sorted(conf_out["outfiles_ts"].items()):
        timings_i = read_identify_write_core(
                conf_in, conf_out, conf_topo, conf_preproc,
                conf_idfy, conf_split, conf_exe, lon, lat,
                infiles_per_outfile, conf_exe["timings"],
                timesteps_out__outfile
            )

def read_identify_write_par(conf_in, conf_out, conf_topo, conf_preproc,
        conf_idfy, conf_split, conf_exe, lon, lat, infiles_per_outfile,
    ):
    ...
    n_chunks = num_procs
    chunks_launches = [[[] for _ in range(n_chunks)]
    ...
    print("\n"+("+"*40))
    print(("parallelize {} outfiles over {} launches with {} threads "
            "(max. {} each)").format(n_outfiles_tot, n_launches,
            num_procs, max_per_proc))
    print(("+"*40)+"\n")

    # Run in parallel
    fct = functools.partial(read_identify_write_par_core,
            conf_in, conf_out, conf_topo, conf_preproc, conf_idfy, conf_split,
            conf_exe, lon, lat, infiles_per_outfile, timings_clean)
...
def read_identify_write_par_core(conf_in, conf_out, conf_topo,
        conf_preproc, conf_idfy, conf_split, conf_exe, lon, lat,
        infiles_per_outfile, timings, outfiles_ts
    ):
    for timesteps_out__outfile in outfiles_ts:
        timings_i = read_identify_write_core(
                conf_in, conf_out, conf_topo, conf_preproc,
                conf_idfy, conf_split, conf_exe, lon, lat,
                infiles_per_outfile, timings, timesteps_out__outfile
            )
    return timings

def read_identify_write_core(conf_in, conf_out, conf_topo, conf_preproc,
        conf_idfy, conf_split, conf_exe, lon, lat,
        infiles_per_outfile, timings, timesteps_out__outfile
    ):
...
def relate_files_ts(files1_ts, files2_ts):
    ...
    for tss1, file1 in files1_ts.items():
        files2_file1[file1] = [file2 for tss2, file2 in files2_ts.items()
                if any(ts1 in tss2 for ts1 in tss1)]
    return files2_file1

#==============================================================================

def identify_features(*, name, conf_in, conf_topo, conf_preproc, conf_idfy,
        conf_split, lon, lat, timestep, timings_measure,
        grid=None, silent=False, its=None, nts=None
    ):
    timings = {}
    ...
    if conf_in["comp_special"] in ["cyclones", "anticyclones"]:
        ...
        flds_named, new_features = identify_cyclones(infile, name,
                conf_in, conf_preproc, timestep,
        ...
        if conf_in["comp_special"] == "fronts":
            ...
            flds_named = compute_front_fields(infile, conf_in, conf_preproc,
                    conf_in["conf_comp_fronts"])
        else:
    ...
    if conf_idfy["grow_features_n"]:
        ...
        features_grow(conf_idfy["grow_features_n"], new_features, const,
                inplace=True, retain_orig=True)
...
def import_fields(infile, lon, lat, conf_in, conf_preproc):
    ...
    fld = read_field(infile, fld_name, level, transpose, fld_mirror,
            informat, reduce_stride, reduce_mode, lon, lat)
    flds_named = {fld_name: fld}

    # Masking
    mask_name   = conf_preproc["mask_varname"]
    if mask_name:
        fld_mask = read_field(infile, mask_name, level, transpose,
                conf_preproc["mask_mirror"], informat, reduce_stride,
                reduce_mode, lon, lat)
        mask_field(fld, fld_mask, conf_preproc["mask_threshold_gt"],
    ...
    if replace_varname:
        ...
        transpose   = conf_in["infield_transpose"]
        mirror      = False
        fld_repl = read_field(infile, replace_varname, level, transpose,
                mirror, informat, reduce_stride, reduce_mode, lon, lat)
...
def compute_front_fields(infile, conf_in, conf_preproc, conf_comp_fronts):
    ...
    if informat != "field":
        err = ("input format must be 'field' to compute fronts, not '{}'"
                ).format(informat)
        raise ValueError(err)

    # Read raw input fields
    iflds = fronts_read_raw_fields(infile, level,
            var_sfx=conf_comp_fronts["var_sfx"])
    ...
    _fmt = lambda s: "{}_{}{}".format(s,
            conf_comp_fronts["tvar"],
            ''#conf_comp_fronts["var_sfx"]
        )
    ...
    if mask_name:
        ...
        if mask_mirror:
            fld_mask = -fld_mask
        mask_field(fld, fld_mask, mask_threshold_gt,
                mask_threshold_lt)
...
def read_field(infile, varname, level, transpose, mirror, input_format="field",
        reduce_stride=1, reduce_mode="mean", lon=None, lat=None):
    ...
    if infile.endswith(".npz"):
        ...
        try:
            ...
            err = ("error reading field '{}' from file '{}':\n{}({})"
                    ).format(varname, infile, e.__class__.__name__, e)
            raise Exception(err)
...
def identify_cyclones(infile, name, conf_in, conf_preproc, timestep,
        anti=False):
    ...
    fld = read_input_field_lonlat(infile, fld_name, level, "lon", "lat",
            conv_fact=fact, crop=conf_preproc["crop_domain_n"])
    ...
    if anti:
        ...
        if refval is None:
            err = ("must provide reference value to identify anticyclones "
    ...
    features = []
    cyclones_to_features(timestep, cyclones, fld, fld.lon, fld.lat, out=features, vb=False)

    flds_by_name = {fld_name: fld}
    return flds_by_name, features

def read_input_field_lonlat(input_file, fld_name, level,
        lon_name, lat_name, conv_fact=None, crop=0):
    ...
    try:
        ...
        err = "Cannot read '{}' from {}\n{}: {}".format(fld_name,
                input_file, e.__class__.__name__, str(e).strip())
...
try:
    ...
    msg = ("warning: fronts-related import failed: {}; cannot identify fronts!"
            ).format(e)
    print(msg)
    def parser_add_group__comp_fronts(*args, **kwas): pass
    def preproc_args__comp_fronts(*args, **kwas): pass
#SR_TMP>
...
def parser_extend_group__in(parser, group):
    ...
    group.add_argument(
            "--fld-mirror",
            help    = ("mirror the input field values during the "
    ...
    group.add_argument(
            "--fld-nomirror",
            help    = ("don't mirror the input field values during the "
                        "identification step"),
            action  = "store_false",
            default = False,
            dest    = "in__infield_mirror",
    ...
    group.add_argument(
            "--reduce-grid-mode",
            help    = ("how to reduce the values from the original to the "
                        "strided grid"),
            metavar = "mode",
            choices = ("pick", "mean",),
            default = "mean",
    ...
    group.add_argument(
            "--replace-varname",
            help    = ("name of variable which replaces feature values after "
    ...
    group.add_argument(
            "--comp",
            help    = ("compute front fields from raw COSMO output fields "
    ...
    parser_add_group__comp_fronts(parser, "compute fronts",
            preflag="fronts", name="in__comp_fronts")
    #--------------------------------------------------------------------------
    return group
...
def parser_add_group__preproc(parser):
    ...
    group.add_argument(
            "--mv", "--mask-varname",
            help    = ("name of variable used for masking; if passed, the "
    ...
    group.add_argument(
            "--mask-gt",
            help    = ("lower threshold for masking; pass lower and/or upper "
    ...
    group.add_argument(
            "--mask-lt",
            help    = ("upper threshold for masking; pass lower and/or upper "
                        "threshold for masking"),
            type    = float,
            metavar = "threshold",
    ...
    group.add_argument(
            "--mask-and-or",
            help    = ("whether both ('and') or only one ('or') threshold "
    ...
    group.add_argument(
            "--reference-value",
            help    = ("reference value for input field, necessary for "
    ...
    group.add_argument(
            "--add-reference-value",
            help    = ("add reference value to fields, which can be useful "
...
def parser_add_group__idfy(parser):
    ...
    group.add_argument(
            "-t", "--type-code",
            help    = ("3-digit feature type codes for feature id; can be "
...
def preproc_args__in__base(parser, kwas, conf):
    ...
    infiles_tss = TimestepStringFormatter(conf.pop("infile_format")
            ).run(conf["timesteps"])
...
def preproc_args__out(parser, kwas, conf):
    ...
    outfiles_ts = TimestepStringFormatter(conf.pop("outfile_format")
...
def preproc_args__idfy(parser, kwas, conf):
    ...
    if type_code is not None:
        if type_code < 0 or type_code > 999:
            parser.error(("invalid type code {} for {}; must be in range "
                    "[0, 999]").format(feature_name, type_code))
            raise ValueError(err)
    else:
        try:
            type_code = DEFAULT_TYPE_CODES[feature_name]
        except KeyError:
            parser.error(("no default feature type code found for {}; please "
                    "specify type codes or add new default").format(
                        feature_name))
...
def preproc_args__split(parser, kwas, conf):
    ...
    if split_levels and len(split_levels) > 1:
        parser.error(("cannot accept split levels: {}\n\n"
